...
usage() { echo "Usage: sudo -v ; curl https://raw.githubusercontent.com/delhi2050/rclone/mod/install.sh | sudo bash" 1>&2; exit 1; }

if [ -n "$1" ]; then
  tag_name="$1"
else
  tag_name=$(curl -fsS https://api.github.com/repos/delhi2050/rclone/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
fi
...
download_link="https://github.com/delhi2050/rclone/releases/download/${tag_name}/${rclone_zip}"
